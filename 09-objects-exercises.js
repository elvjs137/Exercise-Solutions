...
const people = [me, person1, person2];
...
console.log(greetPerson({ firstName: "Tim", lastName: "Johns" }));
...
const enoughSun = (num, plant) => num >= plant.sunlight;
...
const resetDate = (obj) => ({ ...obj, date: Date.now() });
...
const validFarmAnimal = (obj) => {
  const keys = Object.keys(obj);
  return (
    keys.includes("species") && keys.includes("age") && keys.includes("owner")
  );
};
...
const analyzeWardrobe = (wardrobe) => {
  ...
  const tops = wardrobe.tops.length,
    pants = wardrobe.pants.length,
    shorts = wardrobe.shorts.length,
    skirts = wardrobe.skirts.length;

  const actualNumberOfOutfits = tops * pants + tops * shorts + tops * skirts;

  if (actualNumberOfOutfits >= wardrobe.desiredNumberOfOutfits)
    return `Your desired number of outfits works!`;
  else
    return `You need to add more clothing for your desired number of outfits to work.`;
};
...
const makeAnimal = (name) => {
  const animals = ["horse", "cow", "chicken", "lamb"];
  const randomIndex = Math.floor(Math.random() * 2) + 1;
  const randomAge = Math.floor(Math.random() * 10) + 1;

  return {
    species: animals[randomIndex],
    age: randomAge,
    name: name,
  };
};

console.log(makeAnimal("Zeus"));
console.log(makeAnimal("Maya"));
console.log(makeAnimal("XX-099-671"));
